...
def bowling(balls):
    ...
    def advance():
        curr = balls[i]
        score[0] += curr
        score[0] += curr * strike[0]
        strike[0], strike[1] = strike[1], 0
        score[0] += curr * spare[0]
        spare[0] = 0
        return curr
    while i < len(balls):
        s1 = advance()
        if s1 == 10:
            # strike
            strike = [strike[0]+1, strike[1]+1]
            if frame == 10:
                score[0] += strike[0] * balls[i+1]
                score[0] += strike[1] * balls[i+2]
                break
        else:
            i += 1
            s2 = advance()
