...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    ...
    if init == goal:
        return 0
    dimx, dimy = len(road), len(road[0])
    for i in range(dimx):
        cost.append([0]*dimy)
    #op = [(1./road[init[0]][init[1]], init)]
    op = [(0, init)]
    expanded = []
    found = False
    count = 0
    while not found and op and count < 10000:
        cost, exp = op.pop(0)
        x, y = exp
        expanded.append((x, y))
        for dx, dy, lcc in [(-1, 1, lane_change_cost), (0, 1, 0), (1, 1, lane_change_cost)]:
            nx, ny = x + dx, y + dy
            if 0<=nx and nx < dimx and 0<=ny and ny < dimy and (nx, ny) not in expanded:
                cst = cost + 1./(road[nx][ny] + 1e-10) + lcc
                op.append((cst, [nx, ny]))
        op.sort()
        nxt = op[0]
        found = (nxt[1] == goal)
